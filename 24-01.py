        ...
        if self.new == None:
            ...
            if len(self.now.edges) > 1:
                ...
                if self.grid[self.now.edges[p].x][self.now.edges[p].y] == 3:
                    ...
                    self.new = self.now
                    self.grid[self.carx][self.cary] = 3
                elif self.now.traffic is not True and (((self.now.edges[p].timer < 100 or self.now.edges[
                    p].timer >= 185) and self.now.edges[p].timer != -1 and self.now.x == self.now.edges[p].x) or (
                            self.now.edges[p].timer >= 85 and self.now.edges[p].timer != -1 and self.now.y ==
                    self.now.edges[p].y)):
                    self.now.edges.append(self.old)
                    self.new = self.now
                    self.grid[self.carx][self.cary] = 3
